...
use embedded_hal::{
    blocking::{ spi::{ self, Transfer }, i2c, delay::DelayMs },
        ...
        CS: OutputPin<Error = PinErr>
{
    ...
    pub fn is_connected(&mut self) -> Result<bool, Error<SpiErr, I2cErr, PinErr>> {
        ...
        if
            (test_value == result && chipid == valid_ov2640_chipid1) ||
            chipid == valid_ov2640_chipid2
        {
            Ok(true)
        } else {
            Ok(false)
    ...
    fn send_resolution(&mut self) -> Result<(), Error<SpiErr, I2cErr, PinErr>> {
        match self.resolution {
            ...
            Resolution::Res1024x768 => {
                self.sensor_writeregs8_8(&OV2640_1024x768_JPEG)?;
            }
            Resolution::Res1280x1024 => {
                self.sensor_writeregs8_8(&OV2640_1280x1024_JPEG)?;
            }
            Resolution::Res1600x1200 => {
                self.sensor_writeregs8_8(&OV2640_1600x1200_JPEG)?;
            }
            Resolution::Res176x144 => {
                self.sensor_writeregs8_8(&OV2640_176x144_JPEG)?;
            }
            Resolution::Res320x240 => {
                self.sensor_writeregs8_8(&OV2640_320x240_JPEG)?;
            }
            Resolution::Res352x288 => {
                self.sensor_writeregs8_8(&OV2640_352x288_JPEG)?;
            }
            Resolution::Res640x480 => {
                self.sensor_writeregs8_8(&OV2640_640x480_JPEG)?;
            }
            Resolution::Res800x600 => {
                self.sensor_writeregs8_8(&OV2640_800x600_JPEG)?;
            }
        }
...
impl<SPI, I2C, CS> fmt::Debug
    for Arducam<SPI, I2C, CS>
    where SPI: fmt::Debug, I2C: fmt::Debug, CS: fmt::Debug
{
